...
@include('homepage.homepageHeader')
 ...
 <section class="post-services-section" id="post-services">
  ...
  <div class="post-services-carousel" id="servicesCarousel">
    @php
      // Get ONLY posts with ratings (exclude any with 0 ratings)
        $ratedPosts = $posts->where('average_rating', '>', 0)
                            ->where('review_count', '>', 0);
                            
        // Get top rated posts (with rating >= 3.5)
        $topRatedPosts = $ratedPosts->where('average_rating', '>=', 3.5)
                                  ->sortByDesc('average_rating')
                                  ->take(8);
        
        // If not enough highly rated posts, fill with other rated posts (but still with ratings > 0)
        if ($topRatedPosts->count() < 5) {
          $additionalPosts = $ratedPosts->whereNotIn('id', $topRatedPosts->pluck('id'))
                                    ->sortByDesc('average_rating')
                                    ->take(5 - $topRatedPosts->count());
          $topRatedPosts = $topRatedPosts->concat($additionalPosts);
        }
    @endphp
    ...
    @forelse ($topRatedPosts as $post)
      <div class="post-card" data-category="{{ $post->freelancer->categories->pluck('id')->first() }}">
        <div>
          <!-- Top-Rated Badge for 4.5+ rated professionals -->
